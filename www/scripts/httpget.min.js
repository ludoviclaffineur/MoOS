...
function rateGrid(form){
    var rateValue;
    for (var i = form.length - 1; i >= 0; i--) {
        if (form[i].id == "rate"){
            rateValue = form[i].value;
        }
    };
    if (rateValue!=""){
        var xmlHttp = null;
        xmlHttp = new XMLHttpRequest();
        xmlHttp.open( "GET", "rateGrid.snf?rate="+rateValue, false);
        xmlHttp.send( null );
        init();
    }
}
...
function kymaOutput(form){
    if(form[0].id =="ipAddress"){
        var ipAddress = form[0].value;
        var xmlHttp = null;
        xmlHttp = new XMLHttpRequest();
        xmlHttp.open( "GET", "kymaOutput.snf?ipAddress="+ipAddress+"&", false);
        xmlHttp.send( null );
        sleep(400);
        init();
    }
